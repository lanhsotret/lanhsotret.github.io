...
const moveStyle = (function() {
  ...
  function transcountinue(btn1Target, btn2Target, ele) {
    ...
    eleTarget.forEach((particle, index)=>{
      if(eleTarget.length - 2 <= index && index < eleTarget.length) {
        particle.style.transform = `translateX(-${transMove * eleTarget.length}px)`;
      }else{
    ...
    btn1.addEventListener("click", ()=>{
      ...
      eleTarget.forEach((particle, index)=>{
        if(count - 2 <= index && index < count) {
          ...
          particle.style.transform = index >= eleTarget.length - 2?`translateX(-${transMove * Math.abs(index - (eleTarget.length - 2) + num--)}px)`: `translateX(${transMove * Math.abs(index - (eleTarget.length - 2) + num--)}px)`;
        }else{
          particle.style.transition = "transform 0.5s ease";
          particle.style.transform = `translate(${parseFloat(particle.style.transform.split("(")[1]) - transMove * 2}px)`;
        }
      });
      eleTarget[0].addEventListener("transitionend", function observer(){
        btn1.disabled = false;
        eleTarget.forEach((particle,index)=>{
          if(count - 2 <= index && index < count)
            particle.style.visibility = "visible";
        });
        eleTarget[0].removeEventListener("transitionend", observer);
      });
    });
    btn2.addEventListener("click", ()=>{
      btn2.disabled = true;
      ...
      eleTarget.forEach((particle, index)=>{
        if(count <= index && index < count + 2) {
          particle.style.transition = "none";
          particle.style.visibility = "hidden";
          particle.style.transform = `translateX(-${transMove * (index + num--)}px)`;
        }else{
          particle.style.transition = "transform 0.5s ease";
          particle.style.transform = `translate(${parseFloat(particle.style.transform.split("(")[1]) + transMove * 2}px)`;
        }
  ...
  return {translate: translate, translate2: translate2, transcountinue: transcountinue};
})();
...
moveStyle.transcountinue(".sideshow-editorChoice__button.div-button__bt1", ".sideshow-editorChoice__button.div-button__bt2", ".sideshow-editorChoice__div");
moveStyle.transcountinue(".sideshow-filmSoon__button.div-button__bt1", ".sideshow-filmSoon__button.div-button__bt2", ".sideshow-filmSoon__div");
moveStyle.transcountinue(".sideshow-latest__button.div-button__bt1", ".sideshow-latest__button.div-button__bt2", ".sideshow-latest__div");
