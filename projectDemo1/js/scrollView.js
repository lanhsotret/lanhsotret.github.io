...
(function ($) {
  ...
  let count = -1,
      timeDelay = 100,
      ArrayClass = ['zoom', 'fadeUP100', 'effect'];
        ...
        if (wasInView === undefined) {
          ...
          if(elTop < viewportBottom) {
            if(eleTarget.data('function')) {
              eleTarget.data('function')('#' + eleTarget.children('svg').attr('id'), eleTarget.data('variety'));
              $.each(eleTarget.children('.label-chart'), function(index) {
                let _this = $(this);
                setTimeout(function() {
                  _this.removeClass('showup');
                }, index * 200);
              })
            }else {
              if(eleTarget.data('odometer')){
                initOdometer(eleTarget);
              } else {
